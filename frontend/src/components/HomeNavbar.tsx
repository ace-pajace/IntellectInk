...
const HomeNavbar: React.FC<NavbarProps> = ({ children }) => {
    ...
    const [navItems, setNavItems] = useState(children.map((title, idx) => ({ title, isActive: idx === 0 })));
    ...
    return (
        ...
        >
            {
                children.length == 0 ? (
                    <Flex bg="pink.500" px={5} py={4} w="100%">
                        <Text fontSize="lg" fontWeight="bold" color="white" onClick={() => navigate('/courses')}>IntellectInk - Home Page</Text>
                    </Flex>
                ) : (
                    <>
